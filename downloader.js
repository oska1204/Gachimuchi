...
const isWin = flags.includes('--platform=windows') || require('./node_modules/youtube-dl/lib/util.js').isWin

// First, look for the download link.
let dir, filePath
const defaultBin = path.join(__dirname, '.', 'node_modules/yt-dlp')
const defaultPath = path.join(defaultBin, 'details')
const url = process.env.YOUTUBE_DL_DOWNLOAD_HOST || 'https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp'
...
function createBase (binDir) {
  ...
  filePath = path.join(dir, exec('yt-dlp'))
}
...
function downloader (binDir, callback) {
  ...
  download(url, function error (err, newVersion) {
    ...
    return callback(null, 'Downloaded yt-dlp ' + newVersion)
  })
}
